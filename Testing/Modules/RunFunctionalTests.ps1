    ...
    .DESCRIPTION
        ...
        using the following naming convention:
            - SettingsExport.json renamed to ProviderExport-<tennant>
            - TestResults.json renamed to TestResults-<tennant>

        EXAMPLE
            - ProviderExport-contoso
            - TestResults-contoso
    ...
    param (
        ...
        [Alias('t')]
        [string]$IntegrationTest = 'regression',
...
function Compare-TestResults {
    ...
    $SavedFilename = Get-NewFilename -FilePath $GoldenRegressionResultFilename -RegressionResultDirectory $RegressionResultDirectory
    ...
    if (Confirm-FileExists $SavedFilename) {
        ...
        if (($GoldenRegressionResultJson | ConvertTo-Json -Compress) -ne ($TestResultJson | ConvertTo-Json -Compress)) {
            ...
            $ResultString = "`n`t$(Split-Path -Path $GoldenRegressionResultFilename -Leaf -Resolve) : DIFFERENT"
        }
        else {
            $ResultString = "`n`t$(Split-Path -Path $GoldenRegressionResultFilename -Leaf -Resolve) : CONSISTENT"
        }
    }
    return $ResultString
}
...
function Get-NewFilename {
    ...
    return Join-Path -Path (Get-Item $RegressionResultDirectory) -ChildPath $NewFilename
}
...
Set-Location $(Split-Path -Path $PSScriptRoot | Split-Path)
    ...
    switch ($IntegrationTest) {
        'regression' {
            ...
            elseif ($GoldenTestsDirectory -eq '') {
        ...
        Default {
            Write-Error "Unknown test type: '$IntegrationTest'"
        }
    }